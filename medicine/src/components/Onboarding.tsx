    ...
    if (step < totalSteps) {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          alert(response.data.message || "Failed to save profile data");
        }
      } catch (error: any) {
        console.error('Error saving onboarding data:', error);
        alert(error.response?.data?.message || "Error saving profile data. Please try again.");
      }
      setLoading(false);
    }
  };
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`p-4 rounded-xl font-semibold transition-all text-left ${
                          patientData.wearableType === device.name
                            ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg'
                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                        }`}
                      >
                  ...
                  {commonConditions.map((condition) => (
                    <button
                      ...
                      className={`p-3 rounded-xl text-sm font-semibold transition-all ${
                        patientData.chronicConditions.includes(condition)
